...
// Additional medication operations
...
export async function updateMedicationSupply(
  ...
  if (medication) {
    medication.currentSupply = Math.min(
      Math.max(newSupply, 0),
...
export async function refillMedication(
  ...
  if (medication) {
    medication.currentSupply = Math.min(
      medication.currentSupply + refillAmount,
      medication.totalSupply
    );
    medication.lastRefillDate = new Date().toISOString();
    await updateMedication(medication);
  }
}
// In your storage file
export async function getDosesForMedication(
  medicationId: string
): Promise<DoseHistory[]> {
  const history = await getDoseHistory();
  return history.filter((dose) => dose.medicationId === medicationId);
}

export async function clearOldDoses(daysToKeep: number = 30): Promise<void> {
  const history = await getDoseHistory();
...
export const deleteDose = async (doseId: string): Promise<void> => {
  try {
    const history = await getDoseHistory();
    const newHistory = history.filter((dose) => dose.id !== doseId);
    await AsyncStorage.setItem(DOSE_HISTORY_KEY, JSON.stringify(newHistory));
  } catch (error) {
